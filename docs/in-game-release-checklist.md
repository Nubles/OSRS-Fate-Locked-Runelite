...
- Use a tracker profile linked to the character you log in with. Have a
  second character ready for row 10.
- Start from a RuneLite profile with no Fate Locked rules loaded, and turn
  ...
  default) so rows 4 and 5 can show them.
...
| 9 | With Strict Mode on and rules less than 15 minutes old, cast a teleport to a locked destination. Then try **Walk here**, an NPC, a door, a bank and **Wear** on locked ground. | The teleport is blocked, with a chat line and an entry under Recent prevented actions. Nothing else is blocked. | |
...
| 12 | Paste a rules export made more than 15 minutes ago. | It imports; Strict Mode reads Inactive ("the rules are more than 15 minutes old"). | |
